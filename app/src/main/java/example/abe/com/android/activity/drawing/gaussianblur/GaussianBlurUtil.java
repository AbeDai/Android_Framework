...
/**
 * Created by abe on 16/12/14.
 */

import android.graphics.Bitmap;

final public class GaussianBlurUtil {
     ...
     * @param original         原始图
     * @param radius           模糊半径
     * @param canReuseInBitmap 是否重用原始图
     * @return 高斯模糊图片
     */
    ...
    private static Bitmap buildBitmap(Bitmap original, boolean canReuseInBitmap) {
        // 原始图是否存在
        if (original == null)
            throw new NullPointerException("原始图不存在");

        Bitmap.Config config = original.getConfig();
        if (config != Bitmap.Config.ARGB_8888 && config != Bitmap.Config.RGB_565) {
            throw new RuntimeException("图片格式不支持高斯模糊");
        }
